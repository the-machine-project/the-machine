...
public class KillAllCommand extends Command {
    ...
    public KillAllCommand(IdentityDataBaseFile idbf) {
        super(idbf, "killall", Permissions.PermissionLevel.KIND_REGULAR);
    }
    ...
    public ArrayList<ArrayList<TextFragment>> getHelpOutput() {
        ...
        arr.add(new TextFragment("'killall' -- Exits the entire machine (including the terminal). Usage:", Color.WHITE));
        ArrayList<TextFragment> arr2 = new ArrayList<>();
        arr2.add(new TextFragment("     killall", Color.WHITE));
        output.add(arr);
        output.add(arr2);
        return output;
    }
